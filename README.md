...
Details: eval/results/generation_ablation_summary.md
...
Details: eval/results/rerank_ablation_summary.md
...
Details: agentic_rl/results/bm25_budget_controller_summary.md
...
1. 扩充 hardcase source-level labels。
2. 区分 retrieval miss、rerank failure 和 generation unsupported claim。
3. 加强 citation coverage 与 unsupported claim analysis。
4. 将 adaptive retrieval budget controller 保留为 appendix，只有在更大样本和更强 state features 下稳定超过 fixed baseline 时，再升级为主实验结论。
