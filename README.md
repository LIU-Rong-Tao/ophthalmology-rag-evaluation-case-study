...
- Vision caption hard 生成对比：`eval/results/vision_caption_hard_generation_summary.md`
...
- Metadata-aware source evaluation patch：`patches/metadata_aware_source_evaluation.patch`
...
在 8 个图表相关问题上，在 hard vision caption set 上，metadata-aware evaluator 显示 source_hit_rate 从 text-only baseline 的 `0.0000` 提升到 caption-augmented 的 `1.0000`。生成层面，图表关键词平均覆盖从 `9.38` 提升到 `10.12`，说明 caption 入库显著改善了图表来源召回，生成收益则相对温和，仍需要更严格的 image/table-only golden set 继续验证。
